...
def run_ebm_model(n_zones, albedo, solar_constant, emissivity, sigma, n_iter, plot_type):
    """
    ...
    try:
        ...
        if plot_type == "收斂曲線 (Convergence Curve)":
            ...
            for i in range(n_zones):
                ax.plot(iterations, zone_temps_history[:, i], label=f'Zone {i+1}', alpha=0.7)
...
with gr.Blocks(
    ...
    **Absorbed Solar Radiation = Outgoing Longwave Radiation**
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            n_zones = gr.Slider(
                ...
                label="Number of Climate Zones",
                info="Number of climate zones to simulate"
            )
            ...
            plot_type = gr.Radio(
                ...
                info="Choose how to visualize the results"
            )
    ...
    run_button.click(
        fn=run_ebm_model,
        inputs=[n_zones, albedo, solar_constant, emissivity, sigma, n_iter, plot_type],
